...
export function IceBreakersManager() {
    ...
    const handleAdd = () => {
        if (breakers.length >= 4) { toast.error(t("icebreakers.maxError")); return }
    ...
    const handleRemove = (index: number) => setBreakers(breakers.filter((_, i) => i !== index))

    const handleSave = async () => {
        if (!userId) return
        if (breakers.some(b => !b.question?.trim() || !b.response?.trim())) {
            toast.error(t("icebreakers.fillError")); return
        }
        setSaving(true)
        try {
            ...
            if (data.success) toast.success(t("icebreakers.saved"))
            else toast.error(t("icebreakers.saveFailed"))
        } catch { toast.error(t("icebreakers.saveError")) }
    ...
    return (
        <div className="space-y-6 max-w-2xl mx-auto">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-semibold tracking-tight text-foreground">{t("icebreakers.title")}</h2>
                    <p className="text-muted-foreground text-sm mt-0.5">{t("icebreakers.subtitle")}</p>
                </div>
                <Button
                    onClick={handleSave}
                    disabled={saving}
                    className="bg-accent-yellow text-accent-yellow-foreground hover:opacity-90 font-medium text-sm shadow-sm"
                >
            ...
            <div className="space-y-3">
                {breakers.map((item, idx) => (
                    <div key={idx} className="bg-card border border-border p-4 rounded-xl space-y-3 group">
                        <div className="flex justify-between items-start gap-4">
                            <div className="flex-1 space-y-3">
                                <div>
                                    <label className="text-[11px] text-muted-foreground font-medium">{t("icebreakers.question")}</label>
                                    <Input
                                        value={item.question}
                                ...
                                <div>
                                    <label className="text-[11px] text-muted-foreground font-medium">{t("icebreakers.autoResponse")}</label>
                                    <Textarea
                                        value={item.response}
            ...
            <div className="bg-muted border border-border p-4 rounded-xl flex gap-3 text-sm text-foreground">
                ...
                <p className="text-xs text-muted-foreground">{t("icebreakers.syncNote")}</p>
            </div>
        </div>
    )
}
